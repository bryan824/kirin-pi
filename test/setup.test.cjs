...
test("setup CLI is one global command with no required flags", () => {
  assert.deepEqual(parse(["setup"]), { help: false, dryRun: false, home: os.homedir() });
  assert.equal(parse(["setup", "--global"]).help, false);
  assert.equal(parse(["setup", "--dry-run"]).dryRun, true);
  assert.throws(() => parse(["bootstrap", "workflow"]), /Expected `kirin-pi setup`/);
});
...
test("Pi settings preserve unrelated config while disabling package-owned skills", () => {
  const home = tempDir();
  const file = path.join(home, "settings.json");
  write(file, JSON.stringify({
    theme: "dark",
    packages: [KIRIN_SOURCE, "npm:other"],
  }));

  const settings = configurePiSettings(file);
  assert.equal(settings.theme, "dark");
  assert.ok(settings.packages.includes("npm:other"));
  assert.ok(settings.packages.includes("npm:@tintinweb/pi-subagents"));
  assert.ok(settings.packages.includes("npm:pi-web-access"));
  const kirin = settings.packages.find((entry) => entry?.source === KIRIN_SOURCE);
  assert.deepEqual(kirin.skills, []);
  assert.deepEqual(kirin.extensions, ["harness/extensions/*.ts", "harness/extensions/*/index.ts"]);
});

test("settings writes preserve symlinks and restrictive modes", () => {
  const home = tempDir();
  const managed = path.join(home, "managed", "settings.json");
  const link = path.join(home, ".pi", "agent", "settings.json");
  write(managed, JSON.stringify({ packages: [] }));
  fs.chmodSync(managed, 0o600);
  fs.mkdirSync(path.dirname(link), { recursive: true });
  fs.symlinkSync(managed, link);

  configurePiSettings(link);
  assert.equal(fs.lstatSync(link).isSymbolicLink(), true);
  assert.equal(fs.statSync(managed).mode & 0o777, 0o600);
  assert.deepEqual(JSON.parse(fs.readFileSync(managed, "utf8")).packages[0].skills, []);
});
...
test("Claude imports canonical Pi AGENTS while custom text survives", () => {
  ...
  const first = installInstructions(home, "run-1");
  assert.equal(first.backup, undefined);
  assert.equal(fs.readlinkSync(path.join(home, ".claude", "AGENTS.md")), piAgents);
  const claude = fs.readFileSync(path.join(home, ".claude", "CLAUDE.md"), "utf8");
  assert.equal(claude, "@AGENTS.md\n\nCustom Claude note.\n");
  const pi = fs.readFileSync(piAgents, "utf8");
  assert.equal((pi.match(new RegExp(START, "g")) ?? []).length, 1);
  assert.match(pi, /Keep me\./);

  installInstructions(home, "run-2");
...
test("instruction writes preserve dotfile-managed symlinks", () => {
  ...
  installInstructions(home, "run-1");
...
test("dry run mutates neither home nor repository", () => {
  const home = tempDir();
  const before = fs.readFileSync(path.join(root, "AGENTS.md"), "utf8");
  const result = spawnSync(process.execPath, [script, "setup", "--dry-run", "--home", home], {
    cwd: root,
    encoding: "utf8",
  });
  assert.equal(result.status, 0, result.stderr);
  assert.match(result.stdout, /Kirin setup dry run/);
  assert.equal(fs.existsSync(path.join(home, ".pi")), false);
  assert.equal(fs.readFileSync(path.join(root, "AGENTS.md"), "utf8"), before);
});

test("full CLI setup completes against an isolated home", () => {
  const home = tempDir();
  const checkout = path.join(home, ".pi", "agent", "git", "github.com", "bryan824", "kirin-pi");
  const fixture = fixtureCheckout(home);
  fs.mkdirSync(path.dirname(checkout), { recursive: true });
  fs.renameSync(fixture, checkout);
  ...
  const result = spawnSync(process.execPath, [script, "setup", "--home", home], {
    cwd: root,
  ...
  assert.deepEqual(settings.packages.find((entry) => entry?.source === KIRIN_SOURCE).skills, []);
});
