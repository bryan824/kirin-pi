...
| `bun test test/harness.test.cjs test/setup.test.cjs` | Setup resolves interactive choices and `--scope`, `--project`, `--packs`, and `--yes`; global scope installs core only, project scope mirrors selected optional packs into both `.agents/skills` and `.claude/skills`, both scopes stage and roll back skill-tree replacements, and project setup stays inside its selected project root. |
