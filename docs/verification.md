...
| `bun test test/setup.test.cjs` | Zero-argument global setup works with and without Pi, rebuilds both agent skill roots from the package on every run, preserves agent and instruction collisions, never requires project scope, and leaves `~/.pi/agent/settings.json` to `pi install`. |
