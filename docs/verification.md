...
| `bun test test/setup.test.cjs` | Zero-argument global setup works with and without Pi, installs real skill copies into both agent roots, prunes only its own on rerun, preserves collisions, never requires project scope, and leaves `~/.pi/agent/settings.json` to `pi install`. |
