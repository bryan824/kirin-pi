...
When Pi exposes the typed `herdr` tool, prefer it for common actions in its schema: listing, layout creation, pane command execution, reads, waits, focus, input, and cleanup. It preserves aliases and caller context without shell quoting.

The tool is intentionally not a second complete Herdr client. Use the official CLI flow above for agent start/prompt/get, worktrees, integrations, notifications, sessions, or any feature the tool does not expose. The installed CLI remains the syntax and behavior authority.
